...
class CookingEnv:
    ...
    def pickup_obj(self, object_name, mode="closest", verbose=False):
        if mode == "closest":
            ...
            if not obj_found:
                ...
                return False
        event = self.controller.step(
            action='PickupObject', objectId=obj_id, forceAction=True, manualInteract=False)
        return event.metadata["lastActionSuccess"]
    ...
    def obj_interact(self, object_name, action, mode="closest", verbose=False):
        ...
        if mode == "closest":
            ...
            if not event.metadata["lastActionSuccess"]:
                print("Action {} on object {} failed due to: ".format(
                    action, object_name), event.metadata["errorMessage"])
    ...
    def put_obj_backup(self, objectId):
        ...
        distances = [np.linalg.norm(pos_dict_to_array(pos) - obj_pos)
        ...
        for i in sorted_indices:
            ...
            if success:
                ...
                print("Backup put failed at position: {} due to: {}".format(
    ...
    def generate_possible_actions(self, return_language_tags=False):
        ...
        if return_language_tags:
            action_language_tags = [self.action_language_templates[action["action"]].format(
                **self.sanitize_object_names(action)) for action in possible_actions]
            ...
            for obj in objects:
                if obj["objectType"] == condition["object"]:
                    ...
                    if condition["relation"] == "receptacle_contains":
                        receptacle_object_types = [objectID.split(
                            "|")[0] for objectID in obj["receptacleObjectIds"]]
    ...
    def generate_language_predicates(self):
        ...
        for obj in objects:
            if obj["isPickedUp"]:
                predicates.append(
                    "currently holding {}".format(obj["objectType"]))
            if obj["isCooked"]:
            ...
            if self.obj_compare(obj, self.receptacles) and len(obj["receptacleObjectIds"]) > 0:
                receptacle_object_types = [objectID.split(
                    "|")[0] for objectID in obj["receptacleObjectIds"]]
                predicates.append("{} contains: {}".format(
    ...
    def load_task_state(self, task_dict, action_index):
        ...
        for action in pre_task_actions:
            ...
            success = self.parse_action(action)
            if not success:
                print("Error loading task state, pre task action {} failed".format(
                    action["action"]))
                return False
        ...
        for i in range(action_index):
            if action_segments[i]["thor_object"]:
                ...
                success = self.parse_action(action)
                if not success:
                    print("Error loading task state, action {} object {} at index {} failed".format(
