...
class Trie(object):
	def __init__(self, sequences: List[List[int]] = [], bos: any = None, eos: any = None):
		self.trie_dict = {}
		self.len = 0
	...
	def get(self, prefix_sequence: List[int]):
		ret, assert_failed =  Trie._get_from_trie(
			prefix_sequence, self.trie_dict, self.append_trie, self.bos_token_id, self.eos_token_id 
		)
		if assert_failed:
			print("assert failed")
			print("assert prefix sequence: ", prefix_sequence)
	...
	def _get_from_trie(
		...
		if trie_dict=={}:
			try:
				assert(len(prefix_sequence) == 0)
			except AssertionError:
				print("prefix sequence: ", prefix_sequence) 
				assert_failed = True
			return eos_token_id, assert_failed
		...
		if len(prefix_sequence) == 0:
			...
			return output, assert_failed
		elif prefix_sequence[0] in trie_dict:
			ret, assert_failed_temp =  Trie._get_from_trie(
				prefix_sequence[1:],
				trie_dict[prefix_sequence[0]],
				append_trie,
				bos_token_id,
			)
			return(ret, assert_failed or assert_failed_temp)
		else:
			if append_trie:
				return append_trie.get(prefix_sequence), assert_failed
			else:
...
def get_query_trie(tokenizer, queries):
	queries = [e.strip() for e in queries]
	tokened_queries = tokenizer(queries)["input_ids"]
	# print("tokened_queries: ", tokened_queries)
	return Trie(tokened_queries, tokenizer.bos_token_id, tokenizer.eos_token_id), tokened_queries

def filter_phrases_by_trie(len_input, query_trie, tokenizer, verbose = False):
	def filter_trie(batchId, inputIds):
		# print("Input tokens: ", tokenizer.batch_decode(inputIds[len_input:], skip_special_tokens=False))
		if inputIds[len_input:] in query_trie.sequences:
			return tokenizer.eos_token_id
		options = query_trie.get(inputIds[len_input:].tolist())
		# print("Options: ",options)
		# if options and 0 in options:
		#     print("oh no")
		#     print("inputIds: ", inputIds[len_input:])
		ret =  options if options else [tokenizer.eos_token_id]
		if verbose:
			print("Input tokens: ", tokenizer.batch_decode(inputIds[len_input:], skip_special_tokens=False), "option tokens: ", tokenizer.batch_decode(ret, skip_special_tokens=False))
		return(ret)
