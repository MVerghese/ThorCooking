...
with open('constraint_action_full_test_2.csv', 'w', newline='') as csvfile:
...
for current_df in tqdm(grouped_annotations):
    start_idx = 0
    end_idx = start_idx
    while start_idx < len(current_df):
...
Generated constraint dictionary from the above actions:'''
        ...
        with open('constraint_action_full_test_2.csv', 'a', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
...
with open('constraint_action_full_test.csv', 'w', newline='') as csvfile:
    fieldnames = ['narration_id','participant_id','video_id','narration_timestamp','start_timestamp','stop_timestamp',
                  'start_frame','stop_frame','constraint']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
