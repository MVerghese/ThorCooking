...
class Grounding:
    ...
    def __init__(self, load_lavila=True, lavila_cuda_device=0):
        if load_lavila:
            self.lavila = LaViLa_Interface.LaViLa_Interface(
                cuda_device=lavila_cuda_device
            )
        self.language_tags = []
    ...
    def set_language_tags(self, language_tags):
        ...
        self.language_tags_embedding = (
            self.lavila.eval_text(language_tags).cpu().numpy()
        )

    def compute_similarity(self, video_path, frame_range):
        frame_nums = np.linspace(
            frame_range[0], frame_range[1], self.frames_per_eval
        ).astype(int)
...
def main():
    ...
    language_tags = [
        ...
        "add bread to plate",
        "spread mayonnaise",
        "add the bacon to the sandwich",
        "slice tomato",
        "add tomato to sandwich",
        "slice lettuce",
        "add lettuce to sandwich",
        "add bread to sandwich",
    ]
    print(language_tags)
