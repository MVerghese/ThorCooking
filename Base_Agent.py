...
class Base_Agent:
    def __init__(self, language_model=LLAMA_PATH, load_llm=True, load_narrator=True):
        if load_llm:
            self.llm = transformers_interface(
                model_name=LLAMA_PATH, cuda_devices=[0])
        if load_narrator:
    ...
    def narrate_video_unsegmented(self, video_path, narrate_frequency, narrations_per_clip=1):
        ...
        for i in range(0, num_frames, frames_per_segment):
            frame_nums = np.linspace(
                i, min(i+frames_per_segment, num_frames-1), self.frames_per_clip, dtype=int)
    ...
    def summarize_narrations(self, narrations, goal=None, token_limit=4096):
        ...
        while num_prompt_tokens > token_limit:
            ...
            prompt = "A person is currently attempting to " + goal + \
    ...
    def group_and_summarize_narration_history(self, task, narration_history):
        ...
        for narration_block in narration_history:
            narration_embeddings = self.text_embedd_model.encode(
                narration_block)
        ...
        summarized_narrations = self.summarize_narrations(
