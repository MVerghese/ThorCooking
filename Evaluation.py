...
def single_eval(agent, task, environment, start_index, num_gt_actions, timeout=10):
    env = Environment.setup_environment(
        "FloorPlan" + str(environment), task, start_index, history="gt"
    )
    ...
    try:
        ...
        for i in range(num_actions):
            ...
            action, probs, index = agent.select_action(
                task,
...
def full_eval(task, environments, start_indices, evals=5, selection_strategy="sim"):
    agent = Base_Agent.Base_Agent(
        load_llm=True, load_narrator=False, selection_method=selection_strategy
    )
    ...
    for i in range(len(environments)):
        for j in range(len(start_indices)):
            ...
            for k in range(evals):
                task_success, _ = single_eval(
                    agent, task, environments[i], start_indices[j], 8, timeout=7
                )
