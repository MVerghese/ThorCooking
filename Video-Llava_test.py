...
class VLM_Interface:
	def __init__(self, model_path="/home/mverghese/Models/Llama-3.2-11B-Vision-Instruct/", system_prompt_file = ""):
		...
		if system_prompt_file:
			...
			self.system_prompt = "You are an assitant designed to help people with household tasks. Do not be verbose. Answer the question with no added qualifications or caveats. Just directly provide the answer."
	...
	def gen_chat_inputs(self, prompt, image):
		sys_message = {
			"role": "system",
	...
	def generate(self, prompt, image, max_new_tokens=80, stop_strings = None):
		inputs = self.gen_inputs(prompt, image)
		generate_ids = self.model.generate(**inputs, max_new_tokens=max_new_tokens, eos_token_id=self.eos_token_id, tokenizer = self.processor.tokenizer, stop_strings = stop_strings)
	...
	def generate_chat(self, prompt, image, max_new_tokens=80):
		inputs = self.gen_chat_inputs(prompt, image)
	...
	def yes_no_question(self, image, prompt):
		inputs = self.gen_inputs(prompt, image)
	...
	def SeViLa_VQA(self, query, frame_range, video_path, num_query_frames = 16):
		...
		answer_confidences = [self.yes_no_question(clip[i], localizer_prompt)[0] for i in range(num_query_frames)]
		...
		answer = self.generate(question_prompt, clip[np.argmax(answer_confidences)], stop_strings = ". ").split(". ")[0] +"."
...
def conformal_prediction_with_task_dict(MMLLM, task_dict, video_path, alpha=0.1, verbose=False, filter_options=False):
...
if __name__ == '__main__':
	...
	VLM = VLM_Interface()
	video_path = "Videos/make_a_blt_0.mp4"
	# # frame = int((1601+2160)/2)
	# frame = 812
	# clip = load_video_frames_cv2(video_path, [frame])[0]
	# print(clip.shape)
	# prompt = "<|image|><|begin_of_text|>Is there mayonnaise on the bread in this image?"
	# # prompt = "Is there mayonnaise on the bread in this image?"
	# yes_prob, no_prob = VLM.yes_no_question(clip, prompt)
	# print(yes_prob)


	frame_range = [2161, 2327]
	frame_range = [1356, 1600]
	answer, best_frame = VLM.SeViLa_VQA("What is the state of the tomato?", frame_range, video_path)
	print("Answer: ", answer)
