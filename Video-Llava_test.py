import numpy as np
from transformers import VideoLlavaProcessor, VideoLlavaForConditionalGeneration, MllamaForConditionalGeneration, AutoProcessor
...
class MMLLM_Interface:
	def __init__(self):
		...
		self.mcq_tokens = [self.A_token_id, self.B_token_id, self.C_token_id, self.D_token_id, self.E_token_id]
	...
	def mcq_question(self, video_frames, prompt, num_options=5):
		...
		# print(logits)
		log_probs = torch.log_softmax(logits, dim=-1).detach()
		probs = [np.exp(log_probs[0][token_id].item()) for token_id in self.mcq_tokens[:num_options]]
	...
	def action_probs_yn(self, video_frames, actions, add_no_option = False):
		...
		probabilities = [self.yes_no_question(video_frames, prompt.replace('[action]',action))[0] for action in actions]
	...
	def bisection_search(self, video_path, target_action, search_range, max_iterations=5, top_k = 5):
		# total_frames = int(get_video_metadata(video_path)['nb_frames'])
		# frame_range = (0, total_frames)
		result_dict = self.bisection_helper(video_path, target_action, search_range, max_iterations)
		best_range = None
		best_prob = 0
	...
	def bisection_helper(self, video_path, target_action, frame_range, max_iterations):
		...
		result_dict = result_dict | self.bisection_helper(video_path, target_action, first_half, max_iterations-1) | self.bisection_helper(video_path, target_action, second_half, max_iterations-1)
		return result_dict
...
def probability_visualizer(result_dict, prob_index, row_scale, cells_per_row, mode="entropy", gt_range = None):
	rows = []
	if mode == "entropy":
		min_entropy = np.inf
		max_entropy = 0
	...
	for key in result_dict.keys():
		...
		if mode == "entropy":
			...
			value = reverse_norm(calc_entropy(probabilities),min_entropy,max_entropy) if np.argmax(probabilities) == prob_index else 0
	...
	for i, row in enumerate(rows):
		for key_dict in row:
			...
			vis_array[i,key_dict['range'][0]:key_dict['range'][1]] = key_dict['value']
	...
	vis_array = cv2.resize(vis_array, (cells_per_row, vis_array.shape[0]), interpolation=cv2.INTER_NEAREST)
...
def test_conformal_prediction(result_dict):
	...
	for key in result_dict.keys():
		...
		print("Range: {}, Probabilities: {}, Prediction Set: {}".format(key, probabilities, prediction_set))

	return qhat

def conformal_prediction_with_task_dict(MMLLM, task_dict, video_path, alpha = .1,verbose = False, filter_options = False):
	
	possible_actions = [segment["action_text"] for segment in task_dict["action_segments"]]
	...
	if 1-np.mean(top_k_accuracy) > alpha:
		print("Top k error {} is greater than alpha {}".format(1-np.mean(top_k_accuracy), alpha))
	...
	for i in range(n):
		...
		prediction_set = np.where(probabilities >= 1 - qhat)[0]
		if verbose:
			print("Range: {}, Probabilities: {}, Prediction Set: {}".format(cal_ranges[i], probabilities, prediction_set))
		predicition_sets.append(prediction_set)
	print("average prediction set size: ", np.mean([len(prediction_set) for prediction_set in predicition_sets]))
